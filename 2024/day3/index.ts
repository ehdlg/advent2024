...
async function getInstructions() {
  ...
  const instructions = getContentsFromFile(FILE_PATH);
...
async function getMultResult() {
  const instructions = await getValidInstructions();

  if (null == instructions) {
    console.error('There are no valid instructions');
  ...
  const result = instructions.reduce((acc, curr) => {
    ...
    if (null != match) {
      return acc + multiply(match.map((number) => +number));
    }
