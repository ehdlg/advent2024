...
async function part1() {
  const safeReports = await getSafeReports();

  console.log(`Safe reports in Part One: ${safeReports.length}`);
}
...
async function part2() {
  ...
  const safeReports = reports.filter((report) => {
    ...
    for (let i = 0; i < report.length; i++) {
      const partialRerport = [...report.slice(0, i), ...report.slice(i + 1)];

      if (isValidReport(partialRerport)) return true;
    }

    return false;
  });

  console.log(`Safe reports in Part Two: ${safeReports.length}`);
}
