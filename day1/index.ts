...
in one of the first fifty places they'll look, so in order to save Christmas, you need to help them get fifty 
...
locations to check is currently empty. Eventually, someone decides that the best place to check first would be the Chief 
Historian's office.
...
planning the Chief Historian was doing before he left. Perhaps these notes can be used to determine which locations to search?
...
export async function getLists() {
  ...
  const SEPARATOR = /\s+/;
  const fileContents = (await getContentsFromFile(FILE_PATH)).split(SEPARATOR);
  const leftList: number[] = [];
  const rightList: number[] = [];

  for (let i = 0; i < fileContents.length; i++) {
    const number = Number(fileContents[i]);
...
async function getTotalDistance() {
  ...
  const listSize = leftList.length === rightList.length ? leftList.length : null;
